...
20. As someone in the foyer, I want the LED to show a QR + instructions when idle, so that I know how to join.
...
22. As a family, I want our Family Reveal to appear when we reach two members and again when we grow, so that we can gather for a photo.
...
32. As an operator, I want `/admin` and `/led` gated by a shared secret, so that only staff reach them while participants stay open.
...
- **Upstash Redis** (Marketplace) is the single source of truth: love-style counters, family records, an append-only submission log consumed by cursor. Flushed after the event.
- The **LED `/led` client polls** a read endpoint every ~1–2s; no WebSockets, no managed pub/sub. Polling doubles as flood control (LED drains the queue at its own pace).
...
- **Membership**: open the whole event; the Family Love Mix recomputes live on each join; **size cap 10**; no self-leave (admin removes).
...
- **Hybrid authority**: Redis holds coarse state (current mode set by admin + the submission queue + aggregates); the `/led` client owns fine-grained reveal timing and queue drain rate.
- **Modes**: Welcome/Idle, Individual Reveal, Family Reveal, Community Dashboard, Photo Moment.
- **Priority**: admin-forced > Photo Moment (admin) > Family Reveal (new/grown) > Individual Reveal > Community Dashboard > Idle.
- **Cadence**: ~1 individual reveal / 4–6s; cut to dashboard ~10–15s after every ~6–8 reveals or ~60s; drop to Idle after ~20–30s quiet. Family reveals throttled to ≤once / 3–5 min per family; near-simultaneous joins coalesced.
...
### Admin auth (ADR-0006)
- Single **shared secret** (env-configured) checked in Routing Middleware, gating `/admin` and `/led`; participant routes stay open. Do **not** use Vercel Deployment Protection (it would lock out participants).
